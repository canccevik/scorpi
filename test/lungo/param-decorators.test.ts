...
describe('[Lungo] Param Decorators', () => {
  ...
  beforeAll(async () => {
    app = await ScorpiFactory.create(LungoAdapter, { controllers: [ParamDecoratorsController] })
  ...
  it('should host decorator work', async () => {
    ...
    expect(res.text).toEqual(`127.0.0.1:${port}`)
  })
